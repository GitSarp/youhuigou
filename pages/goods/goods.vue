<template>
 ...
 -->
				...
				<view class="buy" @tap="copyTpwd">复制淘口令</view>
			</view>
		</view>
		<!-- share弹窗 -->
		...
		<view class="description">
			...
			<view class="content">
				<image :src="descriptionStr"></image>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			//console.log(option.tpwd); //打印出上个页面传递的参数。
			this.goodsData.id = option.numIid;
			this.goodsData.tpwd = option.tpwd;
			this.getGoodsDetail();
		},
		onReady() {
		...
		methods: {
			getGoodsDetail() {
				uni.request({
					...
					success: (res) => {
						res = res.data
						if (res.smallImages && res.smallImages.length > 0) {
							this.swiperList = res.smallImages
							this.goodsData.name = res.title
							this.goodsData.price = res.zkFinalPrice
							this.descriptionStr = res.pictUrl
						}
					}
				})
			},
			...
			copyTpwd() {
				...
				uni.setClipboardData({
					data: this.goodsData.tpwd,
					success: function(data) {
						uni.showToast({
							title: '复制淘口令成功！快去淘宝app购买吧！'
						})
					},
					fail: function(err) {
					}
